...
- `U02a-1` ✅ read-only inventory panel, now the `INVENTORY_SOURCE` config rendered through the generic `QuantityPanel` described below (docked at `#ui-root`'s right edge via a reusable `getDock` helper); icons resolve through `IconResolver`, which crops sprites from the same `world-atlas.png` the canvas loads (no second icon pipeline) and re-exports the atlas descriptors from `@rendering`; unresolvable icons show a dashed "?" placeholder instead of a blank.
- **Generic quantity panel/popup abstraction** ✅ the docked-card + sortable-popup + cheat-editable-row pattern, originally inventory-only, is now generic: `QuantityRows.ts` holds the pure row type, sort, and DOM row builders; `QuantitySource` (`title`, `collectRows`, optional `cheatSetQuantity`/`listAddableRows`/`cheatRemoveRow`, `subscribeRefresh`) is the pluggable contract; `QuantityPanel`/`QuantityModal` are the only card/popup classes — every feature is a `QuantitySource` config, not a new class. Cheat quantity editing (chevrons + click-to-edit), a "＋" add tile, and a "×" remove button all appear only in the popup, gated by `isCheatsEnabled()`. The add tile opens a true floating picker (icon + label per candidate, arrow-key navigable) portalled into the `<dialog>` itself rather than inline, so it is never clipped by the row list's own scrollbar and survives passive background refreshes; picking a candidate seeds it at a shared starting quantity (100). Removal is source-defined, not just "set to zero" — see `U02a-2` below for why that distinction matters. Background refreshes patch row values in place rather than re-sorting so a row can never reorder out from under an in-progress click; only an explicit sort click or reopening resorts. Updates from `tick:after` plus each source's own change event, not per-frame polling. See [Development and debug support](code-architecture.md#development-and-debug-support) for the general cheat convention this follows.
- `U02a-2` ✅ sector natural-resource panel (`SECTOR_NATURAL_SOURCE`): shows the current sector's woodland biomass, local water stock, and known finite reserves — structured sector state, resolved against the catalog for label/unit/icon, never the company inventory. Unsurveyed reserves render visibly dimmed. The add picker only ever offers reserve resourceIds the sector's own `SectorDef.reserves` actually declares (Ashford Valley currently declares none), never arbitrary catalog/inventory resources. Six new `Application` cheat methods back the popup: `cheatSetSectorWoodlandBiomass`/`cheatSetSectorWaterStock`/`cheatSetSectorReserveQuantity` to set a value, and `cheatClearSectorWoodlandBiomass`/`cheatClearSectorWaterStock`/`cheatRemoveSectorReserve` to remove a row entirely — removal resets woodland/water to `null` and deletes the reserve map entry, deliberately not just zeroing, since a `0` reserve still means "known and depleted" while `null`/absent means "not present at all." All six publish `sector-natural:changed`. On a fresh campaign this panel is correctly empty: `M00`'s runtime bootstrap does not yet seed a sector's natural state from its `SectorDef`'s declared initial values (the "Ashford Valley" centre sector's content definition declares 12,000 kg woodland / 20,000 m³ water, but nothing currently copies that into `CampaignState` at campaign creation) — seeding remains deferred to `C02`/`V00` per `M00`'s own follow-up note. `U02a-4` (site/facility detail) remains shelved.
...
- 526 tests passing; 30 test files.
