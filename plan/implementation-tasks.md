...
- **Generic quantity panel/popup abstraction** ✅ the docked-card + sortable-popup + cheat-editable-row pattern, originally inventory-only, is now generic: `QuantityRows.ts` holds the pure row type, sort, and DOM row builders; `QuantitySource` (`title`, `collectRows`, optional `cheatSetQuantity`, `subscribeRefresh`) is the pluggable contract; `QuantityPanel`/`QuantityModal` are the only card/popup classes — every feature is a `QuantitySource` config, not a new class. Cheat quantity editing (chevrons + click-to-edit) appears only in the popup, gated by `isCheatsEnabled()`, and background refreshes patch row values in place rather than re-sorting so a row can never reorder out from under an in-progress click. Updates from `tick:after` plus each source's own change event, not per-frame polling. See [Development and debug support](code-architecture.md#development-and-debug-support) for the general cheat convention this follows.
- `U02a-2` ✅ sector natural-resource panel (`SECTOR_NATURAL_SOURCE`): shows the current sector's innate woodland biomass, local water stock, and known finite reserves — structured sector state, resolved against the catalog for label/unit/icon, never the company inventory. Unsurveyed reserves render visibly dimmed. Cheat editing (popup only) calls three new `Application` methods — `cheatSetSectorWoodlandBiomass`, `cheatSetSectorWaterStock`, `cheatSetSectorReserveQuantity` — each publishing `sector-natural:changed`. On a fresh campaign this panel is correctly empty: `M00`'s runtime bootstrap does not yet seed a sector's natural state from its `SectorDef`'s declared initial values (the "Ashford Valley" centre sector's content definition declares 12,000 kg woodland / 20,000 m³ water, but nothing currently copies that into `CampaignState` at campaign creation) — seeding remains deferred to `C02`/`V00` per `M00`'s own follow-up note. `U02a-4` (site/facility detail) remains shelved.
...
- 508 tests passing; 30 test files.
